 ...
 * and is provided as a convenience for implementers working with coverage information
 ...
 *      calculations.
 * @param {Boolean} options.skipSourceMaps - true to indicate that source map processing
 *      should be skipped. Used internally to provide a reporting collector that
 *      itself already has source maps processed.
 */
function Collector(options) {
    options = options || {};
    this.store = options.store || new MemoryStore();
    this.reportStore = null;
    this.skipSourceMaps = options.skipSourceMaps;
    this.cache = sourceMaps.createCache();
...
Collector.prototype = {
    ...
    /**
     * disposes the report store if one exists
     * @method disposeReportStore
     * @private
     */
    disposeReportStore: function () {
        if (this.reportStore) {
            this.reportStore.dispose();
            this.reportStore = null;
        }
    },
    /**
     * returns the reporting view as a collector that has
     * source maps processed.
     * @return {Collector} a collector with a collection of files after
     *  source maps have been processed
     */
    reportView: function () {
        var that = this,
            cache = sourceMaps.createCache(),
            t = null;

        if (this.skipSourceMaps) {
            return this;
        }
        if (!this.reportStore) {
            this.files().forEach(function (key) {
                var fileCoverage = that.fileCoverageFor(key);
                if (fileCoverage.sourceMappingUrl) {
                    cache.addUrl(key, fileCoverage.sourceMappingUrl);
                }
            });
            if (cache.hasMappings()) {
                ...
                t = cache.transformer();
                this.files().forEach(function (file) {
                    t.addFileCoverage(file, that.fileCoverageFor(file));
                });
                this.reportStore = t.done();
            } else {
                return this;
            }
        }
        return new Collector({ store: this.reportStore, skipSourceMaps: true });
    }
};
